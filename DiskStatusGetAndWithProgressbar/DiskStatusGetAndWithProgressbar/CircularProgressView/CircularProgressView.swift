...
class ProgressBar: UIView {
  ...
  override func draw(_ rect: CGRect) {
    ...
    guard layer.sublayers == nil else {
      ...
      let lineWidth = min(frame.size.width, frame.size.height) * 0.05
    ...
    backgroundLayer = createCircularLayer(strokeColor: backgroundCircleColor.cgColor, fillColor: fillColor.cgColor, lineWidth: lineWidth)
      ...
      // for middle
    progressLayer1 = createCircularLayer(strokeColor: foregroundCircleColor.cgColor, fillColor: fillColor.cgColor, lineWidth: min(frame.size.width, frame.size.height))
    ...
    gradientLayer.frame = self.bounds
      //FIXME: should look at here
      gradientLayer.mask = progressLayer1
